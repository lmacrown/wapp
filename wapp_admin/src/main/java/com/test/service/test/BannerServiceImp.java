...
public class BannerServiceImp implements BannerService {
    ...
    public void deleteBanner(int banCount) {
        ...
        bannerDao.deleteBanner(banCount);//강의리스트삭제
    }

    @Override
    public BannerDto selectBanner(int banCount) {
        BannerDto banner = bannerDao.selectBanner(banCount);
        return banner;
    }
    ...
    public void editBanner(int banCount,  String banContent, String banTitle,  String banImg) {
        ...
        bannerDao.editBanner(banCount,  banContent, banTitle,  banImg);
    }


    @Override
    public void addBanner(int banCount, String banTitle, String banContent, MultipartFile banImg) {
        //강의리스트 추가.(강의 카테고리, 강의이름, 강의 가격, 강의 이미지 매개변수 받아오기)
        //System.out.println("service: " + lecCategory +" "+ lecName +" "+ lecPrice);
        bannerDao.addBanner(banCount,  banTitle,  banContent, banImg);//강의리스트 추가.
    }
